...
async function createCoverPage(
    ...
    // should change this to be more descriptive
    if (includeImage) {
...
// Main function that generates combined PDF
async function generatePDF(questions: Question[]) {
    try {
        ...
        // Add instructions
        await addInstructionsPage(combinedPdf, font, boldFont);

        // Question pages
        let currentPage = combinedPdf.addPage();
        ...
        function checkPageBreak(spaceNeeded: number = lineHeight) {
            if (yPosition < margin + spaceNeeded) {
                currentPage = combinedPdf.addPage();
        ...
        async function fetchAndEmbedImage(url: string) {
            try {
                ...
                if (contentType?.includes('image/png') || url.toLowerCase().includes('.png')) {
                    embeddedImage = await combinedPdf.embedPng(imageBytes);
                } else if (contentType?.includes('image/jpeg') || contentType?.includes('image/jpg') ||
                    url.toLowerCase().includes('.jpg') || url.toLowerCase().includes('.jpeg')) {
                    embeddedImage = await combinedPdf.embedJpg(imageBytes);
                } else {
                    try {
                        embeddedImage = await combinedPdf.embedJpg(imageBytes);
                    } catch {
        ...
        // PART 2: ANSWER BOOKLET
        // Create cover page for answers without image
        ...
        console.log("Downloading combined exam PDF...");
        const pdfBytes = await combinedPdf.save();
        downloadPDF(pdfBytes, 'exam-complete.pdf');
